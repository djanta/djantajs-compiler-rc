...
module.exports = class RuntimeCompiler extends Handler {
  ...
  //  * @return {RuntimeCompiler} return the bundle self instance
  //  */
  // _withContribution (component, file) {
  //   let self = this;
  //   if (component && 'contribution' === component.annotationName) {
  //     let ser = _.merge(component.serialize || {}, { class: file });
  //     self.bundle.contributions = utils.concat(self.bundle.contributions, ser);
  //   }
  //   return self;
  // }
  ...
  //  * @return {RuntimeCompiler} return the bundle self instance
  //  */
  // _withRuntimeLifecycle (component, file) {
  //   let self = this;
  //   if (component && 'runtime' === component.annotationName) {
  //     let ser = _.merge(component.serialize || {}, { class: file });
  //     self.bundle.lifecycles = utils.concat(self.bundle.lifecycles, ser);
  //   }
  //   return self;
  // }
  ...
  //  * @return {RuntimeCompiler} return the bundle self instance
  //  */
  // _withController (component, file, cwd = __dirname) {
  //   let self = this;
  //   if (component && !!~['controller'].indexOf(component.annotationName)) {
  //
  //     let fd = path.join(cwd, file);
  //
  //     /*switch (component.annotationName) {
  //       case 'controller':*/
  //         (self.build.controllers = self.build.controllers || {})[fd] =
  //           _.defaults({
  //             component: component,
  //             class: file
  //           });
  ...
  //  * @private
  //  */
  // _withInterceptor (component, file, cwd = __dirname) {
  //   let self = this;
  //   if (component && !!~['interceptor'].indexOf(component.annotationName)) {
  //     /*switch (component.annotationName) {
  //       case 'interceptor':*/
  //         (self.build.interceptors = self.build.interceptors || {})[path.join(cwd, file)] = {
  //           component: component,
  //           class: file
  //         };
  //       /*break;
      ...
      switch(scope) {
        case 'controllers':
          Object.keys(self._build[scope]).forEach((location) => {
              ...
              Object.keys(xconfig.methods).forEach((method) => {
                if(xconfig.methods[method].verb) { // only if the method has also been annotated with @verb.
                  ['hook', 'request', 'response'].forEach(nested => {
                    ...
                    if (!_.isNil(xvalue)) {
                      xconfig.methods[method].verb.addFeature(nested, xvalue);
                    }
                  });
            ...
            (self.bundle.resources = self.bundle.resources || [])
              .push(_.merge({}, ctrl.serialize, { class: xconfig.class }));
          });
        break;
        case 'interceptors':
          _.each(self._build[scope], (s) => {
