...
let { fill, annotations, required, isSerializable } = require('../../utils');
let _ = require ('lodash');

/**
 * Interceptor annotation serializer.
 * @type {Interceptor}
 */
module.exports = class Interceptor extends Serializable {
  ...
  /**
   * Gets the annotation defined name.
   * @return {string} Returns the annotation given name
   */
  get annotationName() {
  ...
  /**
   * Get the interceptor environment setting name
   * @return {{}} the target environment setting name
   */
  get routine () {
    return this._routines = (this._routines || {});
  }

  /**
   * Gets the interceptor default invocation configuration
   * @param {{}} routine the interceptor default invocation configuration
   */
  set routine (routine) {
    this.routine[routine.annotationName] = routine;
  }
  ...
  get serialize () {
    ...
    _.each(self.routine || {}, (routine, name) => {
      if (isSerializable(routine)) {
        cumulator[name] = routine.serialize;
      }
    });
