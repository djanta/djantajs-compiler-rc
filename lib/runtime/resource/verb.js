...
module.exports = class Verb extends Serializable {
  ...
  /**
   * Register the given feature object.
   * @param {string} name the feature given name
   * @param {object} o the feature given instance
   */
  addFeature(name, o) {
    let self = this;
    ...
    // let now register the given feature ...
    self._features[name] = [...self._features[name] || [], o];
  }
  ...
  get serialize() {
    ...
    fill(self, stash, [
      { name: 'name', render: (o, name) => o[name] || o.target }, 'label', {
        name: 'url', alias: 'urls', render: (o, name) => {
          return _.reduce(_.isArray(o[name]) ? o[name] : [o[name]],
            (result, url) => {
    ...
    if (!_.isNil(self._features) && !_.isEmpty(self._features)) {
      _.each(self._features, (feature, name) => {
        // console.log('============ Name:%s\n', name);
      });
