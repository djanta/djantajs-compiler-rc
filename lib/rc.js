...
module.exports = class RC extends Handler {
      ...
      switch (scope) {
        case 'controllers':
          Object.keys(self._build[scope]).forEach((location) => {
            ...
            (self._bundle.resources = self._bundle.resources
  ...
  handle(options /*resources = [], buildOutputDirectory, outputFile*/) {
    let self = this;
    (options.resources || []).forEach((resource) => self._configure(resource.directory || __dirname)
      ._analyze(resource.project, resource.src || resource.location, resource.directory)
    );
