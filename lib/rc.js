...
const {
  Handler
} = require('djantajs-compiler-core');
...
module.exports = class RC extends Handler {
  ...
  _withBundle(bundle) {
    ...
    if(!_.isNil(bundle)  && 'bundle' === bundle.annotationName) {
      if(!_.isNil(bundle.name)) { self._bundle.name = bundle.name; }
      if(!_.isNil(bundle.version)) { self._bundle.version = bundle.version; }
      if(!_.isNil(bundle.description)) { self._bundle.description = bundle.description; }
      if(!_.isNil(bundle.homepage)) { self._bundle.homepage = bundle.homepage; }
      if(!_.isNil(bundle.author)) { self._bundle.author = bundle.author; }
      if(!_.isNil(bundle.order)) { self._bundle.order = bundle.order; }
      if(!_.isNil(bundle.enabled)) { self._bundle.enabled = bundle.enabled; }
      if(!_.isNil(bundle.tags)) { self._bundle.tags = bundle.tags; }
      if(!_.isNil(bundle.imports)) { self._bundle.imports = bundle.imports; }
    }
    return self;
  }
  ...
  _configure(directory = __dirname) {
    ...
    if(!_.isUndefined(self._bundle)) { return self; } //Already built!
